...
HTML = os.path.join(BASE, "research-video-rec.html").replace("\\", "/")
FPS = 24
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # 冒烟：只截若干关键时刻，供人工检查视觉
        if SMOKE:
            marks = [0.02, 0.14, 0.29, 0.42, 0.58, 0.72, 0.86, 0.97]
            for r in marks:
                ms = int(total * r)
                await page.evaluate("window.__seek(%d)" % ms)
        ...
        await browser.close()
    print("frames done: %d, cost %.1fs" % (N_FRAMES, time.time() - t0), flush=True)
