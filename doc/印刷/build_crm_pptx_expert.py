# -*- coding: utf-8 -*-
"""构建企业AI销售决策平台介绍 PPT（专家版 · 通用 B2B 销售案例 · ~19 页）
四幕闭环：客户痛点 → 问题分析 → 解决方案（企业AI销售决策专家专家包 + 7 方法论 + 真实案例）
           → 客户价值（闭环量化 + 落地路径）
"""
...
txt(s,0.9,1.9,11,1.4,'企业AI销售决策专家',46,WHITE,True)
...
txt(s,6.5+0.3,2.85,5.0,0.5,'企业AI销售决策专家',20,CYAN,True)
...
# =====================================================================
s = newslide()
header(s,'03 · 解决方案','企业AI销售决策专家（专家包）','在 WorkBuddy 里一键召唤，让系统替销售做判断、做闭环')
caps = [
...
steps = [
    ...
    ('安装「企业AI销售决策专家」','搜索安装，一键召唤，开启专属对话',AMBER),
...
txt(s,0.9,2.5,11.5,1.0,
    '先装「企业AI销售决策专家」，用 4 周跑通「一般商机全自动闭环」——\n自动接诊、标准报价、自动跟进，让销售立刻减负。',
...
txt(s,0.9,5.45,11.5,1.2,
    '企业AI销售决策专家 —— 不是把销售搬上线，而是让销售管理成为增长引擎。',
    18,AMBER,True)

# ---------- 保存 ----------
out = '企业AI销售决策平台介绍_2026.pptx'
try:
    prs.save(out)
    print('SAVED_OK', out, 'slides=', len(prs.slides._sldIdLst))
except PermissionError:
    out2 = '企业AI销售决策平台介绍_2026_tmp.pptx'
    prs.save(out2)
