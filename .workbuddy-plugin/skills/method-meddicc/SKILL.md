...
| Action | 用途 |
|---|---|
| `crm-deal-advance` | MEDDICC 硬维度过闸后推进商机阶段 |
| `data-particle-update` | 七维评分写回商机（经第0闸） |

> 写清单一侧为「方法论结果 → 业务写」联动面：本 method SKILL 不直接 dispatch 写 Action，一律经 crm-write 两阶段（第0闸 + action-confirm）执行。七维（M/E/D/C/I/C/C）未过闸禁止乐观推进报价。
