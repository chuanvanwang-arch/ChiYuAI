...
| Action | 用途 |
|---|---|
| `crm-proposal-write` | gate=FAIL 时触发技术方案补写（售前方案产出） |
| `data-particle-update` | 方案要点/技术匹配度写回商机 |

> 写清单一侧为「方法论结果 → 业务写」联动面：本 method SKILL 不直接 dispatch 写 Action，一律经 crm-write 两阶段（第0闸 + action-confirm）执行。gate=FAIL 禁止推进报价，先经 crm-proposal-write 补方案。
