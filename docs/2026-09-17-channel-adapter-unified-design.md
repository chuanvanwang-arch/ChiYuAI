...
> **入口同源要求**：同一失败会在多个入口出现（向导 `onboarding-guide.html` / 配置台 `channel-config.html`），两处 `guidedHint` 必须**逐字一致**，由 `test/web/channelHintParity.test.js` 严格相等守卫（已变异自证：任改一处措辞即红）。理由：一处给真因、一处只报错误码，会让人误以为"某个页面查得更准"，实为其中一个入口吞了归因。
