...
> **版本** v1.0（FINAL，唯一有效设计）｜ **日期** 2026-09-15 ｜ **状态**：待用户批准（P8）  
> **性质**：实施级设计。批准后唯一入口为 `writing-plans`；**批准前不写任何实现代码（HARD-GATE）**。
>
...
| # | 改动 | 位置 | 收益 |
| - | --- | --- | --- |
| **1** | 分级配置纳入 `POLICY_KEYS` | `policyVersion.js:29-34` | 分级变更 → 新版本；历史决策依据**永久可复现**（修 D3，最严重的一条） |
| **2** | 把 `decision_id` 写进分级行 | `businessTier.js:143`（新增列 + 入参） | "这条分级是谁批的"**可反查**（修 D2 溯源断链） |
| **3** | `autonomous_allowed` 接入判定，或从配置面移除 | `autonomyEngine.js:274`（或 `decisionScenario.js:126`） | 消除"配了⛔人工却仍自动放行"的**假绿**（修 E1） |
...
> **A3 的取舍说明（须报批）**：`business_tier_config` 是**表**而非 `config_store` 键，而 `loadSnapshot`（`policyVersion.js:46-53`）只读 `config_store`。故 A3 有两种实现，**须用户裁决**：
> - **方案 i（键镜像，推荐）**：把分级表内容镜像为 `config_store['business-tier-config']` 只读快照，纳入 `POLICY_KEYS`。改动小、与既有 9 键范式一致。
> - **方案 ii（扩快照源）**：`loadSnapshot` 增加"表型配置源"抽象（`{type:'table', table, tenantCol}`）。语义更正，但触及 C3 冻结内核。
>
> 无论哪种，**都属"改 §10 硬约束之外的决策内核"**，须先获显式批准（见 §18.4）。

### 11.1.1 附带缺陷修法（E1 / E2）

| 缺陷 | 修法 | 不做会怎样 |
| --- | --- | --- |
| **E1** `autonomous_allowed` 假绿 | **二选一，须裁决**：**(a)** 让 `autonomyEngine.js:274` 接入该字段（语义：场景级总闸，关闭则即使 tier 低也升级）；**(b)** 从配置面移除该列的编辑与显示，改为派生展示"按 tier 判定"。**不得维持现状**（配置面承诺不生效 = 假绿） | 管理员配了"⛔人工"却仍被自动放行，是**安全承诺失效** |
...
| **S6** | **让它自己动手**（自治）       | **T21** 分级授权对象化（A 轴补全，**先做**）· **T19** 常驻授权 T1（B/C 轴）                              | S5 | 排名 **8**                                |
