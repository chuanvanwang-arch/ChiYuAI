...
| Action | 用途 |
|---|---|
| `crm-deal-advance` | 决策链打通后才推进商机阶段 |
| `data-particle-update` | 角色地图/立场写回商机 |

> 写清单一侧为「方法论结果 → 业务写」联动面：本 method SKILL 不直接 dispatch 写 Action，一律经 crm-write 两阶段（第0闸 + action-confirm）执行。BLOCKED_DECISION（决策链未打通）禁止推进。
