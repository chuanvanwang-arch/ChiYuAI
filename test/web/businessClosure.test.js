...
const BOARD_SAMPLE = {
  ...
  leadPool: { count: 1, note: '线索=CRM_DEAL 中 stage=lead' },
...
describe('businessClosure 渲染纯函数（浏览器可加载子模块）', () => {
  ...
  test('renderBusinessClosure 渲染 6 阶段卡（客户/商机/报价/合同/订单/回款）', async () => {
    ...
    expect(html).toContain('线索池'); // 线索池标注条
  });
  ...
  test('renderBusinessClosure 空数据降级', async () => {
    ...
    const html = renderBusinessClosure({
      stages: [], leadPool: { count: 0, note: '' }, total: { dealAmount: 0, contractAmount: 0, receivedAmount: 0 },
    });
...
describe('businessBoard 富化端点（TDD handler）', () => {
  ...
  test('createBusinessBoardRouter GET 聚合 6 阶段 + leadPool + total', async () => {
    ...
    const mockItems = [
      ...
      { id: 'd2', type: 'CRM_DEAL', payload: { name: '线索L', amount: 0, stage: 'lead' } },
    ...
    expect(body.leadPool.count).toBe(1); // 仅 stage=lead 的 d2
  ...
  test('buildBoard 纯函数：金额映射 + 回款合并 + leadPool 派生', async () => {
    const { buildBoard } = await import('../../src/portal/businessBoard.js');
    const items = [
      { id: 'd1', type: 'CRM_DEAL', payload: { name: 'X', amount: 100000, stage: 'lead' } },
    ...
    expect(b.leadPool.count).toBe(1); // 仅 d1
