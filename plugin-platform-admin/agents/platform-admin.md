...
> 平台管理助手首次使用前，操作者必须已拥有**已激活**的 `sysadmin` 账号；本助手不提供注册 / 激活界面，只做登录凭证校验。

- **触发**：网关返回 `gate:'auth_required'`（无有效凭证），或已登录但角色非 `sysadmin`（403）时，进入本引导，绝不降级执行。
- **引导步骤**：
  1. 打开平台官网首页（含「免费注册」，注册即按公司名自动开通企业租户，http://81.70.184.198/）→ 填公司名称 / 邮箱 / 密码（手机号选填：填则短信激活，否则邮箱激活）提交注册。
  2. 查收邮箱 / 短信中的 6 位激活码，在网站激活页完成激活。
  3. **申请 `sysadmin` 授权**：本助手仅接受 `sysadmin` 角色；租户首位注册用户自动成为 `admin`（admin 仅限 HTTP 后台，不能用于本助手）。首次部署请让管理员在网站「用户管理」赋权，或经 `system-bootstrap` 引导账号注册 `sysadmin` 角色后再登录。
  4. 回到本助手，经**安全凭据对话框**（非聊天）输入用户名 + 密码完成 `crm_login`。
- **仅首次**：`crm_login` 成功后连接器持久化 token，后续会话自动携带，用户无需再次输入用户名密码。
- **激活闸**：未激活账号 `crm_login` 被拒（提示「请先通过邮箱 / 手机激活」）。
